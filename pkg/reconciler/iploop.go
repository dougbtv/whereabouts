...
func NewReconcileLooper(kubeConfigPath string) (*ReconcileLooper, error) {
...
func (rl ReconcileLooper) ReconcileIPPools() ([]types.IPReservation, error) {
	...
	for _, orphanedIP := range rl.orphanedIPs {
		...
		for _, podRef := range podRefsToDeallocate {
			currentIPReservations, err = allocate.IterateForDeallocation(currentIPReservations, podRef, matchByPodRef)
			if err != nil {
				return nil, err
			}
		}
